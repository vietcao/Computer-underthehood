...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: maybe recive a segment fault
  fillArray(array, 10);
  ...
  // Answer: cause array variable is poiter to first element of array itself
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer: the heap segment will be not be remove the extra memory
   free(heap_array);
 ...
 int* heap_four_ints = (int*) malloc(sizeof(FourInts));
 fillArray(heap_four_ints, 4);
 assert(((FourInts*)heap_four_ints)->a == 2);
 assert(((FourInts*)heap_four_ints)->b == 5);
 assert(((FourInts*)heap_four_ints)->c == 8);
 assert(((FourInts*)heap_four_ints)->d == 11);
 free(heap_four_ints);
     return 0;
}
